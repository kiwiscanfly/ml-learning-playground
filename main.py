...
def preprocess_data(df):
  ...
  # df['AgeBin'] = pd.cut(df['Age'].astype(int), bins=[0, 12, 20, 40, 60, np.inf], labels=False)
...
def evaluate_model(model, X_test, Y_test):
  ...
  accuracy = accuracy_score(Y_test, preduction)
  confusion = confusion_matrix(Y_test, preduction)
